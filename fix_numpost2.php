<?php
    ...
    if(isset($_FILES["house_photo"]) && !empty($_FILES["house_photo"]["name"])) {
        ...
        // 如果沒有新的圖片，保持原有的圖片不變
        $sql_select = "SELECT house_photo FROM numbered_post WHERE post_id='$post_id'";
    ...
    // 檢查是否有文章內容發生了改變
    $sql_select = "SELECT * FROM numbered_post WHERE post_id='$post_id'";
    $result = $conn->query($sql_select);
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
    ...
    // 更新文章數據庫中的相應記錄
    $sql = "UPDATE numbered_post SET article='$article', content='$content', star_rate='$star_rate', house_photo='$house_photo' WHERE post_id='$post_id'";
    ...
    <?php
        ...
        <script>
            alert("文章修改失敗!");
            location.href = "my_post_num.php"; // 或者重定向到其他頁面
        </script>
    <?php
    }
